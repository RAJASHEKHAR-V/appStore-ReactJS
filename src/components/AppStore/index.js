...
class AppStore extends Component {
  state = {matchCategoryId: tabsList[0].tabId, userSearchInput: ''}

  onClickOnTabItem = categoryId => {
    this.setState({matchCategoryId: categoryId})
  }
  ...
  filterCategory = () => {
    const {matchCategoryId} = this.state
    const filterList = appsList.filter(
      eachCategoryObject => eachCategoryObject.category === matchCategoryId,
    )
    return filterList
  }

  searchIncludes = filteredCategoryList => {
    const {userSearchInput} = this.state
    const searchedList = filteredCategoryList.filter(eachCategoryObject =>
      eachCategoryObject.appName
        .toLowerCase()
  ...
  render() {
    const {matchCategoryId, userSearchInput} = this.state
    const filteredCategoryList = this.filterCategory()
    const searchFilteredCategoryList = this.searchIncludes(filteredCategoryList)
          ...
          <ul className="tabs-card">
            {tabsList.map(eachTabObject => (
              <TabItem
                ...
                tabItemObject={eachTabObject}
                onClickOnTabItem={this.onClickOnTabItem}
                isTabActive={eachTabObject.tabId === matchCategoryId}
              />
            ))}
          </ul>
          <ul className="category-card">
            {searchFilteredCategoryList.map(eachCategoryObject => (
              <AppItem
                key={eachCategoryObject.appId}
                categoryObject={eachCategoryObject}
              />
            ))}
          </ul>
        </div>
